...
class Transformer {
      ...
      for (const instruction of file.idl) {
        ...
        switch (instruction.type) {
          case "interface":
            ...
            if (this.ctx.suppressErrors && !interfaces.has(instruction.name)) {
              break;
            }
          ...
          case "dictionary":
            ...
            if (this.ctx.suppressErrors && !dictionaries.has(instruction.name)) {
              break;
            }
            ...
            break;
          case "implements":
            if (this.ctx.suppressErrors && !interfaces.has(instruction.target)) {
              break;
            }
  ...
  * _writeFiles(outputDir) {
    ...
    yield fs.writeFile(utilsPath, utilsText);

    if (this.ctx.bundle) {
      const bundleEntryText = yield fs.readFile(path.resolve(__dirname, "output/bundle-entry.js"));
      yield fs.writeFile(path.join(outputDir, "bundle-entry.js"), bundleEntryText);
    }
    ...
    let utilExpr;
    if (this.ctx.bundle) {
      utilExpr = `binding.utils`;
    } else {
      utilExpr = `require("./utils")`;
    }
    ...
    for (const obj of interfaces.values()) {
      ...
      if (this.ctx.bundle) {
        implExpr = `binding.impls.${obj.name}`;
      } else {
        let implFile = path.relative(outputDir, path.resolve(obj.opts.implDir, obj.name + this.implSuffix));
        implFile = implFile.replace(/\\/g, "/"); // fix windows file paths
        if (implFile[0] !== ".") {
          implFile = "./" + implFile;
        }
        implExpr = `require(${JSON.stringify(implFile + ".js")})`;
      }
    ...
    yield fs.appendFile(
      path.join(outputDir, "bundle-entry.js"),
      allExports.map(e => `allExports.${e} = require("./${e}");\n`).join("")
    );
  }
